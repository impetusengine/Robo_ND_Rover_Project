...
def color_rocks(img, rgb_thresh=(110,110,50)): 
    color_select = np.zeros_like(img[:,:,0])
...
def perception_step(Rover):
    ...
    destination = np.float32([[Rover.img.shape[1]/2 - 5, Rover.img.shape[0] - 5], [Rover.img.shape[1]/2+5, Rover.img.shape[0]-5],
                              [Rover.img.shape[1]/2 + 5, Rover.img.shape[0] -10-5], [Rover.img.shape[1]/2-5, Rover.img.shape[0]-10-5]])
    # 2) Apply perspective transform
    warped, mask = perspect_transform(Rover.img, source, destination)
    # 3) Apply color threshold to identify navigable terrain/obstacles/rock samples
    nav_terrain = color_thresh(warped, rgb_thresh = (160,160,160))
    obs_map = color_obstacle(warped, rgb_thresh = (160,160,160))* mask
    rock_map = color_rocks(warped, rgb_thresh = (110,110,50))
    ...
    rocksxpix, rocksypix = rover_coords(rock_map)
    ...
    rock_x_world, rock_y_world = pix_to_world(rocksxpix, rocksypix, Rover.pos[0], Rover.pos[1], Rover.yaw, world_size, scale)
